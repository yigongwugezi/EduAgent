...
from app.rag.chunker import chunk_records, clean_all_records
from app.rag.config import RAGConfig
from app.rag.embedder import create_embedding_model
from app.rag.loader import load_all_records, discover_files
from app.rag.store import create_storage_context, persist_index
...
def build_index(
    ...
    4. Create (or overwrite) FAISS vector store with StorageContext.
    ...
    storage_context = create_storage_context(config, overwrite=True)
    ...
    # Persist both FAISS vectors and document store
    from app.rag.store import _persist_dir

    persist_index(storage_context, _persist_dir(config))
